...
import { Compass, Map, Settings, Menu, X, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function MainLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100 antialiased selection:bg-indigo-500 selection:text-white">
      {/* Sidebar Shell */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-slate-900 bg-slate-950/80 backdrop-blur-md transition-all duration-300 ease-in-out md:static md:translate-x-0 ${
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center rounded-xl py-3 text-sm font-medium transition-all ${
                  isCollapsed ? 'md:justify-center px-0' : 'px-4 gap-3.5'
                } ${
                  isActive
        ...
        <div className={`p-4 border-t border-slate-900 flex transition-all duration-300 ${isCollapsed ? 'md:flex-col md:items-center md:gap-4' : 'items-center gap-3'}`}>
          {user?.avatarUrl ? (
            <img
              src={user.avatarUrl}
              alt={user.name}
              className="h-10 w-10 rounded-full border border-slate-800 object-cover shrink-0"
