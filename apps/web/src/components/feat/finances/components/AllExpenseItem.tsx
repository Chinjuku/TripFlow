...
import type { HydratedExpense, HydratedExpenseSplit } from '../types';
...
const categoryStyles = {
  food: { icon: Utensils, bg: 'bg-purple-100/70 text-purple-700 dark:bg-purple-950/30 dark:text-purple-300' },
...
export function AllExpenseItem({ expense: exp, currentUserId, isExpanded, onToggleExpand }: AllExpenseItemProps) {
  ...
  const splitLabel = exp.split_method === 'equally' ? t('finances.equally', 'Equally') : t('finances.exactAmount', 'Exact Amount');
    ...
    <div className="group flex flex-col bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 ${cat.bg}`}>
      ...
      {isExpanded && (
        <div className="bg-muted/30 border-t border-border px-4 py-4 sm:px-5 space-y-4">
          <div className="flex justify-between items-center text-[10px] font-bold tracking-wider text-muted-foreground uppercase font-label">
            <span>{t('finances.splitBreakdown')} ({exp.splits.length} {t('common.people')})</span>
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    {split.avatarUrl ? (
                      ...
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-primary-foreground shadow-inner ${
                          isPayer ? 'bg-primary' : 'bg-muted-foreground'
                        }`}
                      >
                        {initials}
                      </div>
                    ...
                    <div className="flex flex-col">
                      <span className={`font-medium text-sm ${isSplitMe ? 'text-primary font-semibold' : 'text-foreground'}`}>
