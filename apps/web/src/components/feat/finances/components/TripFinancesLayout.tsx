...
export function TripFinancesLayout({ activeTab, children }: TripFinancesLayoutProps) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-8 h-full lg:h-[calc(100vh-5.5rem)] lg:overflow-hidden">
        <TripPageHeader
          backTo={`/trips/${id}`}
            ...
            {isLoading || !trip || !finances ? (
              activeTab === 'all' ? (
                <div className="flex flex-col flex-1 overflow-hidden gap-6 h-full min-h-0">
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 space-y-4 scrollbar-none">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {[0, 1].map((i) => (
                      <div key={i} className="rounded-2xl border border-border bg-card shadow-sm p-6 flex flex-col h-full min-h-[14rem]">
                        <Skeleton className="h-8 w-32 rounded-full" />
                        <Skeleton className="h-10 w-1/2 mt-3 mb-4" />
                        <div className="space-y-3 flex-1 flex flex-col justify-end">
                          <div className="flex items-center justify-between py-1">
